...
object runEnsembles {
  ...
  def main(arg: Array[String]) {
    ...
    val test = sc.textFile(pathTest, 10).map(line => converter.parserToLabeledPoint(line)).persist
		...
		/*val trainROS = ROS(train, 1.5).persist
    ...
    val modelDT = DecisionTree.trainClassifier(hme_bd, numClasses, categoricalFeaturesInfo, impurity, maxDepth, maxBins)
    ...
    val writer_dt = new PrintWriter(new File("/home/x76654048/results_hmd_filter_dt.txt"))
    writer_dt.write(
    ...
    val modelRF = RandomForest.trainClassifier(hme_bd, numClasses, categoricalFeaturesInfo, numTrees, featureSubsetStrategy, impurity, maxDepth, maxBins)
    ...
    val writer = new PrintWriter(new File("/home/x76654048/results_hmd_filter_rf.txt"))
    writer.write(
      "Precision: " + precision + "\n" +
        "Confusion Matrix " + cm + "\n"
    )
    ...
    val pcardTrain = PCARD.train(hme_bd, trees, cuts)
    ...
    val writer_pc = new PrintWriter(new File("/home/x76654048/results_hmd_filter_pcard.txt"))
    writer_pc.write(
    ...
    writer_pc.close()
		...
		// Naive Bayes
		val model_nb = NaiveBayes.train(hme_bd, lambda = 1.0, modelType = "multinomial")
    ...
    val writer_nb = new PrintWriter(new File("/home/x76654048/results_hmd_filter_bayes.txt"))
    writer_nb.write(
