...
object runEnsembles {
  ...
  def main(arg: Array[String]) {
    ...
    val test = sc.textFile(pathTest, 10).map(line => converter.parserToLabeledPoint(line)).persist
		...
		val trainROS = ROS(train, 1.8).persist
...
/*
		...
		val hme_bd_model = new HME_BD(trainROS, nTrees_hme, partitions_hme, maxDepthRF_hme, 48151623)
...
*/
    ...
    val modelDT = DecisionTree.trainClassifier(fcnn_mr, numClasses, categoricalFeaturesInfo, impurity, maxDepth, maxBins)
    ...
    val writer_dt = new PrintWriter(new File("/home/x76654048/results_ROS_180_fcnn_mr_dt.txt"))
    writer_dt.write(
    ...
    val writer = new PrintWriter(new File("/home/x76654048/results_ROS_180_fcnn_mr_rf.txt"))
    writer.write(
      "Precision: " + precision + "\n" +
        "Confusion Matrix " + cm + "\n"
    )
    ...
    val pcardTrain = PCARD.train(fcnn_mr, trees, cuts)
    ...
    val writer_pc = new PrintWriter(new File("/home/x76654048/results_ROS_180_fcnn_mr_pcard.txt"))
    writer_pc.write(
    ...
    writer_pc.close()
		...
		// Naive Bayes
		val model_nb = NaiveBayes.train(fcnn_mr, lambda = 1.0, modelType = "multinomial")
    ...
    val writer_nb = new PrintWriter(new File("/home/x76654048/results_ROS_180_fcnn_mr_bayes.txt"))
    writer_nb.write(
