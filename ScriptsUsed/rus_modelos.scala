...
object runEnsembles {
  ...
  def main(arg: Array[String]) {
    ...
    val test = sc.textFile(pathTest, 10).map(line => converter.parserToLabeledPoint(line)).persist
		...
		val classInfo = trainRUS.map(lp => (lp.label, 1L)).reduceByKey(_ + _).collectAsMap()
    //Class balance
    ...
    // val writer_dt = new PrintWriter(new File("/home/spark/results_rus_dt.txt"))
    val writer_dt = new PrintWriter(new File("/home/x76654048/results_rus_dt.txt"))
    writer_dt.write(
    ...
    val modelRF = RandomForest.trainClassifier(trainRUS, numClasses, categoricalFeaturesInfo, numTrees, featureSubsetStrategy, impurity, maxDepth, maxBins)
    ...
    //Write Results
    // Para guardar en el cluster
    // val writer = new PrintWriter(new File("/home/spark/results_rus_rf.txt"))
    val writer = new PrintWriter(new File("/home/x76654048/results_rus_rf.txt"))
    writer.write(
      "Precision: " + precision + "\n" +
        "Confusion Matrix " + cm + "\n"
    )
    ...
    val pcardTrain = PCARD.train(trainRUS, trees, cuts)
    ...
    //Write Results
    // Para guardar en el cluster
    // val writer_pc = new PrintWriter(new File("/home/spark/results_rus_pcard.txt"))
    val writer_pc = new PrintWriter(new File("/home/x76654048/results_rus_pcard.txt"))
    writer_pc.write(
    ...
    writer_pc.close()
		...
		// Naive Bayes
		val model_nb = NaiveBayes.train(trainRUS, lambda = 1.0, modelType = "multinomial")
    ...
    //Write Results
    // Para guardar en el cluster
    // val writer_nb = new PrintWriter(new File("/home/spark/results_rus_bayes.txt"))
    val writer_nb = new PrintWriter(new File("/home/x76654048/results_rus_bayes.txt"))
    writer_nb.write(
